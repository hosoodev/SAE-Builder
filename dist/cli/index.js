...
import { build, check, clean as cleanProject, } from "../build/index.js";
...
function printResult(label, pageCount, outputRoot) {
    console.log(`${label}: ${pageCount} page(s) · ${outputRoot}`);
}
function formatDuration(startedAt) {
    return `${Math.max(0, Math.round(performance.now() - startedAt))}ms`;
}
function printDevBuildResult(label, result, startedAt) {
    const { incremental } = result;
    console.log(`[sae] ${label}: ${result.pages.length} page(s) in ${formatDuration(startedAt)}` +
...
function requestPathForLog(requestUrl) {
    try {
...
function printChangedFiles(changedFiles) {
    ...
    console.log(`[sae] Changed: ${visible.join(", ")}${remainder > 0 ? ` (+${remainder} more)` : ""}`);
}
...
async function resolveStaticFile(outputRoot, requestUrl) {
...
async function dev(options) {
    ...
    const watcher = watch(options.root, { recursive: true }, (_event, filename) => {
        ...
        if (isInsideRoot(config.resolvedPaths.output, target) || isInsideRoot(config.resolvedPaths.cache, target))
            return;
    ...
    const server = http.createServer(async (request, response) => {
        ...
        try {
            ...
            console.log(`[sae] ${request.method ?? "GET"} ${requestPathForLog(request.url ?? "/")}` +
    ...
    if (error instanceof BuilderError) {
        ...
        if (Array.isArray(diagnostics)) {
            for (const item of diagnostics) {
                ...
                console.error(`${String(diagnostic.severity ?? "error").toUpperCase()} ${String(diagnostic.ruleId ?? "builder")}` +
