...
import { assertInsideRoot, assertNoSymlinkPath, } from "../filesystem/index.js";
import { DependencyGraph } from "./graph.js";
function compareText(left, right) {
    return left < right ? -1 : left > right ? 1 : 0;
}
export function planInvalidation(previous, currentFingerprints) {
    const changed = new Set();
    if (previous === null) {
        for (const node of Object.keys(currentFingerprints))
            changed.add(node);
    }
    else {
        for (const node of new Set([
            ...
            if (previous.fingerprints[node] !== currentFingerprints[node])
                changed.add(node);
        }
    }
 ...
 */
export async function syncOutputTree(stageRoot, outputRoot, options = {}) {
    const stage = path.resolve(stageRoot);
    const output = path.resolve(outputRoot);
    if (stage === output)
        throw new TypeError("Stage and output roots must differ.");
