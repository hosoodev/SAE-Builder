...
async function assertRootIsDirectory(root: string): Promise<string> {
...
export async function assertNoSymlinkPath(
  ...
  for (const segment of relative.split(path.sep).filter(Boolean)) {
    ...
    try {
      ...
      if (allowMissingLeaf && (error as NodeJS.ErrnoException).code === "ENOENT") break;
      throw error;
    }
  }
  return safe;
...
export async function resolveFileInsideRoot(
  root: string,
  entry: string,
  label = "file entry",
): Promise<string> {
...
export async function discoverFiles(
  root: string,
  directoryOrOptions: string | DiscoverFilesOptions = {},
  maybeOptions: DiscoverFilesOptions = {},
): Promise<string[]> {
...
export async function writeFileAtomic(root: string, target: string, contents: string | Uint8Array): Promise<void> {
...
export async function writeTextInside(root: string, relativePath: string, value: string): Promise<void> {
  await writeFileAtomic(root, relativePath, value);
}

export async function copyDirectory(sourceRoot: string, outputRoot: string): Promise<void> {
