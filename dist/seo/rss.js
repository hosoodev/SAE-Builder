...
export function generateRss(items, options) {
    const resolved = [];
    for (const item of items) {
        ...
        if (item.date)
            validateSeoDate(item.date, `Publication date for ${item.route}`);
        if (item.updated)
            validateSeoDate(item.updated, `Updated date for ${item.route}`);
    ...
    for (const item of resolved) {
        lines.push("    <item>", `      <title>${escapeXml(item.title.trim())}</title>`, `      <link>${escapeXml(item.url)}</link>`, `      <guid isPermaLink="true">${escapeXml(item.url)}</guid>`, `      <description>${escapeXml(item.description.trim())}</description>`);
        if (item.date) {
