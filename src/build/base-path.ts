...
function prefixRootUrl(value: string, basePath: string): string {
...
function rewriteNode(node: HtmlNode, basePath: string): void {
  ...
  for (const child of node.childNodes ?? []) rewriteNode(child, basePath);
  if (node.content) rewriteNode(node.content, basePath);
}

/** Prefix site-local absolute HTML attributes when deploying below an origin base path. */
export function applySiteBasePath(html: string, siteUrl: string | URL): string {
  const basePath = normalizeSiteBase(siteUrl).pathname;
  if (basePath === "/") return html;
  const document = parse(html);
  rewriteNode(document as unknown as HtmlNode, basePath);
