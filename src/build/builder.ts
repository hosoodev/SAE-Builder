...
interface GlobalArtifact extends OutputClaim {
  readonly contents: string;
}

interface PluginArtifact extends OutputClaim {
  readonly contents: string | Uint8Array;
}

interface PageOgArtifact extends OutputClaim {
  readonly route: string;
  readonly url: string;
...
function normalizedOutputPath(relativePath: string): string {
...
function outputComparisonKey(relativePath: string): string {
  return normalizedOutputPath(relativePath).toLowerCase();
}

function assertUniqueOutputOwnership(claims: readonly OutputClaim[]): void {
  ...
  const claimedFiles = new Map<string, (typeof ordered)[number]>();
  ...
  for (const claim of ordered) {
    ...
    if (exact) {
      throw new BuilderError(
        "BUILD_FAILED",
        `Output path collision at ${JSON.stringify(claim.relativePath)} between ${exact.owner} and ${claim.owner}.`,
        {
          details: {
    ...
    for (let length = 1; length < segments.length; length += 1) {
      ...
      if (ancestor) {
        throw new BuilderError(
          "BUILD_FAILED",
          `Output file/directory collision between ${ancestor.owner} (${JSON.stringify(ancestor.relativePath)}) and ${claim.owner} (${JSON.stringify(claim.relativePath)}).`,
          {
            details: {
              outputPath: claim.relativePath,
...
function routeForOutputPath(relativePath: string): string {
...
function manifestUnderSiteBase(manifest: AssetManifest, siteUrl: string): AssetManifest {
...
function stagePathFor(config: ResolvedBuilderConfig): string {
  ...
  const stage = path.join(
    config.resolvedPaths.cache,
    `stage-${process.pid}-${stageSequence}`,
  );
...
export function validateProjectPaths(config: ResolvedBuilderConfig): void {
  ...
  for (const source of sources) {
    if (pathsOverlap(output, source) || pathsOverlap(cache, source)) {
      throw new BuilderError(
        "PATH_INVALID",
        "paths.output and paths.cache must not contain, equal, or be contained by an input root.",
      );
    }
  }
...
function normalizePluginConfig(
  candidate: ResolvedBuilderConfig,
  root: string,
  configFile: string,
...
function assertStablePluginSet(
  before: readonly BuilderPlugin[],
  after: readonly BuilderPlugin[],
): void {
  ...
  if (stableStringify(beforeNames) !== stableStringify(afterNames)) {
    throw new BuilderError(
      "CONFIG_INVALID",
      "The config plugin hook cannot add, remove, or reorder plugins.",
    );
  }
}

function pluginFingerprint(plugin: BuilderPlugin): string {
  const hooks = [
    "config",
    "buildStart",
    "contentLoaded",
...
function normalizedConfigValue(config: ResolvedBuilderConfig): Record<string, unknown> {
  return {
    site: config.site,
    paths: config.paths,
    content: {
...
async function collectBuildFingerprints(
  config: ResolvedBuilderConfig,
  mode: BuildContext["mode"],
  entries: readonly NormalizedContentEntry[],
  assetPlans: readonly AssetBuildPlan[],
  ogArtifacts: readonly PageOgArtifact[],
  ...
  for (const entry of entries) {
    fingerprints[`content:${entry.sourceRelativePath}`] = hashContent(
  ...
  return Object.fromEntries(
    Object.entries(fingerprints).sort(([left], [right]) => compareText(left, right)),
  );
}

function pluginDiagnosticToSeo(
...
function requireAsset(
  ...
  if (!asset) {
    throw new BuilderError(
      "BUILD_FAILED",
      `${source} references unknown ${kind} entry '${name}'.`,
    );
  }
  return asset;
}
...
function renderAssetTags(
  entry: NormalizedContentEntry,
  styleManifest: AssetManifest,
  scriptManifest: AssetManifest,
  ...
  if (fm.faq !== undefined) {
    ...
    const faqItems = fm.faq.map((item, index) => {
      if (
        ...
        throw new BuilderError(
          "BUILD_FAILED",
          `${entry.sourcePath}: faq item ${index + 1} requires question and answer strings.`,
        );
      }
      return {
...
function metadataHeadParts(metadata: ResolvedMetadata): {
  full: string;
  legacyExtra: string;
} {
...
async function renderPage(
  ...
  const outputPath = outputPathForSlug(
    stageRoot,
    entry.route.slug,
    config.build.trailingSlash,
  );
...
async function reusePublishedPage(
  config: ResolvedBuilderConfig,
  entry: NormalizedContentEntry,
  stageRoot: string,
  previousCache: BuildCache,
  generatedImage?: string,
): Promise<RenderedContentPage | null> {
  ...
  if (expectedFingerprint === undefined) return null;
  const publishedPath = assertInsideRoot(
    config.resolvedPaths.output,
    path.join(config.resolvedPaths.output, ...relativePath.split("/")),
  );

  try {
    await assertNoSymlinkPath(config.root, publishedPath);
    const htmlBytes = await readFile(publishedPath);
    if (hashContent(htmlBytes) !== expectedFingerprint) return null;
    return {
      entry,
...
async function recordGraph(
  rendered: readonly RenderedContentPage[],
  ...
  for (const { entry, page } of rendered) {
    ...
    const outputId = `output:${entry.route.slug}`;
    for (const dependency of page.dependencies) graph.addDependency(outputId, dependency);
    ...
    for (const plugin of plugins) graph.addDependency(outputId, `plugin:${plugin.name}`);
    outputs[contentId] = [entry.route.slug];
  }
  return Object.fromEntries(Object.entries(outputs).sort(([a], [b]) => compareText(a, b)));
}

function planGlobalArtifacts(
  config: ResolvedBuilderConfig,
  entries: readonly NormalizedContentEntry[],
  mode: BuildContext["mode"],
): readonly GlobalArtifact[] {
  const artifacts: GlobalArtifact[] = [];
  if (config.seo.sitemap) {
    artifacts.push(...planLocalizedSitemaps(
      entries.map((entry) => ({
        route: entry.route.slug,
        locale: entry.frontmatter.locale ?? config.i18n.defaultLocale,
...
function ogFilenameStem(entry: NormalizedContentEntry): string {
...
async function planPageOgArtifacts(
  config: ResolvedBuilderConfig,
  entries: readonly NormalizedContentEntry[],
  useCache: boolean,
  ...
  for (const entry of entries) {
    ...
    if (requestedTemplate && config.og.templates[requestedTemplate] === undefined) {
      throw new BuilderError(
        "BUILD_FAILED",
        `${entry.sourceRelativePath} references unknown OG template '${requestedTemplate}'.`,
      );
    }
...
async function activateOgFontDiscovery(config: ResolvedBuilderConfig): Promise<() => Promise<void>> {
  if (!config.og.enabled || config.og.fonts.length === 0) return async () => {};

  const fontFiles = await Promise.all(config.og.fonts.map((font) =>
    resolveFileInsideRoot(config.resolvedPaths.templates, font.file, "OG font file")));
  ...
  const xmlPath = (value: string): string => value
...
async function writeAssetPlans(stageRoot: string, plans: readonly AssetBuildPlan[]): Promise<void> {
  const files = plans
    .flatMap((plan) => plan.files)
    .sort((left, right) =>
      compareText(left.relativePath, right.relativePath) || compareText(left.name, right.name),
    );
...
async function writeGlobalArtifacts(stageRoot: string, artifacts: readonly GlobalArtifact[]): Promise<void> {
...
async function writePluginArtifacts(
  stageRoot: string,
  ...
  for (const artifact of artifacts) {
    const target = assertInsideRoot(
      stageRoot,
      path.join(stageRoot, ...artifact.relativePath.split("/")),
    );
...
async function writeOgArtifacts(
  stageRoot: string,
  artifacts: readonly PageOgArtifact[],
): Promise<void> {
  for (const artifact of artifacts) {
    const target = assertInsideRoot(
      stageRoot,
...
function noOutputSync(): OutputSyncResult {
  return { written: [], unchanged: [], removed: [] };
}

function assertContentEntryIdentity(
  before: NormalizedContentEntry,
  after: NormalizedContentEntry,
  hook: "contentLoaded" | "transformContent",
  stageRoot: string,
): void {
  if (
    ...
    throw new BuilderError(
      "PLUGIN_FAILED",
      `${hook} must preserve content source and route identity.`,
      { details: { hook, source: before.sourceRelativePath } },
    );
  }
  ...
  if (after.frontmatter.slug !== after.route.slug) {
    throw new BuilderError(
      "PLUGIN_FAILED",
      `${hook} produced frontmatter.slug that does not match the normalized route.`,
      { details: { hook, source: before.sourceRelativePath } },
    );
  }
}

function assertRenderedPageIdentity(
  before: BuiltPage,
  after: BuiltPage,
  stageRoot: string,
): void {
  if (
    ...
    throw new BuilderError(
      "PLUGIN_FAILED",
      "renderPage must preserve source, route, and outputPath.",
      { details: { hook: "renderPage", source: before.source } },
    );
  }
  if (
    ...
    throw new BuilderError(
      "PLUGIN_FAILED",
      "renderPage returned an invalid page value.",
      { details: { hook: "renderPage", source: before.source } },
    );
  }
...
function assertNoErrorDiagnostics(diagnostics: readonly SeoDiagnostic[]): void {
  const errors = diagnostics.filter(({ severity }) => severity === "error");
  if (errors.length > 0) {
    throw new BuilderError(
      "CHECK_FAILED",
      `SEO/AEO validation failed with ${errors.length} error(s).`,
      { details: { diagnostics } },
    );
  }
}

async function runBuild(options: BuildOptions): Promise<BuildResult> {
  ...
  const pluginRunner = createPluginRunner(config.plugins, {
    ...
    emitFile: async (pluginName, relativePath, contents) => {
      if (!acceptingPluginArtifacts) {
        throw new BuilderError(
          "PLUGIN_FAILED",
          `Plugin '${pluginName}' emitted a file after bundle finalization.`,
          { details: { plugin: pluginName, hook: "buildEnd" } },
        );
      }
      const normalized = normalizedOutputPath(relativePath);
  ...
  try {
    ...
    const baseAssets = Object.fromEntries(
      Object.entries({ ...styleManifest, ...scriptManifest })
    ...
    const contentLoadedEntries: NormalizedContentEntry[] = [];
    ...
    try {
      assertUniqueSlugs(
        entries,
        (entry) => entry.route.slug,
        (entry) => entry.sourceRelativePath,
      );
    } catch (error) {
      throw new BuilderError(
        "BUILD_FAILED",
        error instanceof Error ? error.message : "A plugin produced duplicate content routes.",
        { cause: error },
      );
    }

    const ogArtifacts = await planPageOgArtifacts(config, entries, shouldWrite);
    const ogImageByRoute = new Map(
      ogArtifacts.map((artifact) => [artifact.route, artifact.url] as const),
    );
    const assets: AssetManifest = Object.freeze(Object.fromEntries(Object.entries({
      ...baseAssets,
    ...
    let translationAlternates: ReadonlyMap<string, readonly TranslationAlternative[]>;
    try {
      translationAlternates = createTranslationAlternates(
        entries.map((entry) => ({
          route: entry.route.slug,
          locale: entry.frontmatter.locale ?? config.i18n.defaultLocale,
          translationKey: entry.frontmatter.translationKey,
      ...
      throw new BuilderError(
        "BUILD_FAILED",
        error instanceof Error ? error.message : "Unable to link translated content.",
        { cause: error },
      );
    }

    const currentFingerprints = await collectBuildFingerprints(
      config,
      context.mode,
      entries,
      [stylePlan, scriptPlan],
      ogArtifacts,
      pluginRunner.plugins,
    );
    const previousCache: BuildCache | null = shouldWrite
      ? await loadBuildCache(config.resolvedPaths.cache)
      : null;
    const invalidation = planInvalidation(
      previousCache?.graph ?? null,
      currentFingerprints,
    );
    ...
    const rendered = [] as RenderedContentPage[];
    ...
    for (const entry of entries) {
      ...
      const reused = mayReusePublishedPages
        ...
        ? await reusePublishedPage(
            config,
            entry,
            stageRoot,
            previousCache,
            ogImageByRoute.get(entry.route.slug),
          )
        : null;
      if (reused !== null) {
      ...
      const renderedPage = await renderPage(
        context,
        entry,
        translationAlternates.get(entry.route.slug) ?? [],
        templateLoader,
        stageRoot,
        styleManifest,
        scriptManifest,
    ...
    await pluginRunner.generateBundle({
      ...
      emitFile: async () => {
        throw new BuilderError(
          "PLUGIN_FAILED",
          "Bundle emission requires a plugin-scoped generateBundle hook.",
        );
      },
    });

    const createOutputClaims = (): OutputClaim[] => [
    ...
    const diagnose = (claims: readonly OutputClaim[]): SeoDiagnostic[] => {
      const pageRoutes = new Map(rendered.map(({ entry, page }) => [
        normalizedOutputPath(relativeInsideRoot(stageRoot, page.outputPath)),
        entry.route.slug,
      ]));
      const knownPaths = [...new Set(claims.map(({ relativePath }) =>
        pageRoutes.get(normalizedOutputPath(relativePath)) ?? routeForOutputPath(relativePath)))]
        .sort(compareText);
      return diagnoseSeoSite(
        rendered.map(({ entry, page }) => ({
          route: page.route,
          source: entry.sourceRelativePath,
          html: page.html,
    ...
    const diagnostics: SeoDiagnostic[] = [
      ...
      ...pluginDiagnostics.map((diagnostic) =>
        pluginDiagnosticToSeo(diagnostic, config.lint.warningsAsErrors)),
    ];
    ...
    const resultPages = rendered.map(({ page }) => Object.freeze({
      ...
      outputPath: path.join(
        config.resolvedPaths.output,
        ...relativeInsideRoot(stageRoot, page.outputPath).split("/"),
      ),
    }));
    const incremental = {
    ...
    const provisionalResult: BuildResult = {
      root,
    ...
    for (const diagnostic of diagnostics) {
      context.logger.warn(
        `${diagnostic.ruleId}: ${diagnostic.source ?? diagnostic.route ?? "site"}: ${diagnostic.message}`,
      );
    }

    const outputs = await recordGraph(
      rendered,
      graph,
      currentFingerprints,
      pluginRunner.plugins,
    ...
    if (shouldWrite) {
      ...
      try {
        ...
        context.logger.warn(
          `Build output was published, but the incremental cache could not be saved: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
    }
...
export async function check(options: Omit<BuildOptions, "write"> = {}): Promise<BuildResult> {
  return runBuild({ ...options, write: false });
}

export async function clean(
  options: Pick<BuildOptions, "root" | "configFile"> = {},
): Promise<{ root: string; removed: readonly string[] }> {
  const root = path.resolve(options.root ?? process.cwd());
