...
export function renderIntegrationHead(config: Readonly<IntegrationConfig>): string {
  const lines: string[] = [];
  ...
  if (config.googleAdSense) {
    lines.push(
      `<meta name="google-adsense-account" content="${config.googleAdSense}">`,
    );
  }
  if (config.naverAnalytics) {
    lines.push(
      ...
      `window.wcs_add["wa"] = ${JSON.stringify(config.naverAnalytics)};`,
      "if (window.wcs) {",
      "  window.wcs_do();",
      "}",
      "</script>",
  ...
  if (config.googleAdSense) {
    lines.push(
      `<script async src="https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js?client=${config.googleAdSense}" crossorigin="anonymous"></script>`,
    );
  }
  return lines.join("\n");
}
...
export function generateGoogleAdsTxt(publisherId: string): string {
...
export function daumWebmasterComment(key: string): string {
