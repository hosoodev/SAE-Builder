...
function readNestedValue(data, dottedPath) {
    let value = data;
    for (const segment of dottedPath.split(".")) {
        if (FORBIDDEN_PROPERTY.has(segment))
            return undefined;
...
function stringifyTemplateValue(value, key) {
    ...
    throw new TemplateError("UNSUPPORTED_VALUE", `Template value ${JSON.stringify(key)} must be a primitive value`);
}
function renderVariables(template, data, strictVariables) {
    const read = (key) => {
...
function assertPartialName(name) {
...
async function replacePartials(template, resolver, dependencyIds, chain, maxDepth, expandedCache) {
    let output = "";
    let cursor = 0;
    ...
    for (const match of matches) {
        ...
        if (source === undefined) {
            throw new TemplateError("MISSING_PARTIAL", `Missing partial ${JSON.stringify(partialName)} referenced by ${chain.at(-1) ?? "template"}`);
        }
        const identity = source.dependencyId ?? `partial:${source.name}`;
        if (chain.includes(identity)) {
            throw new TemplateError("PARTIAL_CYCLE", `Partial cycle detected: ${[...chain, identity].join(" -> ")}`);
        }
        if (chain.length >= maxDepth) {
            throw new TemplateError("PARTIAL_DEPTH", `Partial expansion exceeded the maximum depth of ${maxDepth}`);
        }
        dependencyIds.add(identity);
        let expanded = expandedCache.get(identity);
        if (expanded === undefined) {
            expanded = await replacePartials(source.content, resolver, dependencyIds, [...chain, identity], maxDepth, expandedCache);
...
export async function renderTemplate(template, data, options = {}) {
    const dependencies = new Set();
    if (options.rootDependencyId !== undefined)
        dependencies.add(options.rootDependencyId);
    const inlinePartials = options.partials ?? {};
    const resolver = options.resolvePartial ?? ((name) => {
    ...
    const expanded = await replacePartials(template, resolver, dependencies, [rootIdentity], options.maxPartialDepth ?? 64, new Map());
    return {
