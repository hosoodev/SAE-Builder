...
const pluginSchema = z.custom<BuilderConfig["plugins"][number]>((value) => {
  if (!value || typeof value !== "object") return false;
  const plugin = value as { name?: unknown; enforce?: unknown };
...
const collectionDefinitionSchema = z.custom<ContentCollectionDefinition>((value) => {
...
export function resolveConfig(
  input: UserConfig,
  root: string,
  configFile = path.join(root, "builder.config.mjs"),
): ResolvedBuilderConfig {
  ...
  if (explicitSiteDefault && explicitI18nDefault && explicitSiteDefault !== explicitI18nDefault) {
    throw new BuilderError(
      "CONFIG_INVALID",
      "site.defaultLocale and i18n.defaultLocale must agree when both are configured.",
    );
  }
  ...
  if (
    ...
    throw new BuilderError(
      "CONFIG_INVALID",
      "site.locales and i18n.locales must agree when both are configured.",
    );
  }
  ...
  let collections: readonly ContentCollectionDefinition[];
  try {
    collections = normalizeCollectionDefinitions(
      parsed.data.content.collections as CollectionConfig[],
    );
  } catch (error) {
    throw new BuilderError(
      "CONFIG_INVALID",
      error instanceof Error ? error.message : "Invalid content collection configuration.",
      { cause: error },
    );
  }
...
export async function loadConfig(
  ...
  const configFile = (await Promise.all(candidates.map(async (file) => [file, await fileExists(file)] as const)))
    .find(([, exists]) => exists)?.[0];
  if (!configFile) {
    throw new BuilderError(
      "CONFIG_NOT_FOUND",
      `No builder.config.mjs or builder.config.js found in ${normalizedRoot}.`,
    );
  }
  ...
  try {
    ...
    const module = await import(`${pathToFileURL(configFile).href}?v=${fingerprint}`) as {
      default?: UserConfig;
    };
    if (!module.default) {
