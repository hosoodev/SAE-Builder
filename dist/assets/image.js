...
function normalizedWidths(widths, sourceWidth) {
...
export async function optimizeImage(sourcePath, options) {
    ...
    for (const format of formats) {
        for (const width of widths) {
            ...
            const artifact = await getOrCreateArtifact(path.join(options.cacheDirectory, "images"), recipeHash, format, async () => {
...
function variantsFor(result, format) {
...
export function renderPictureHtml(result, options) {
    if (result.variants.length === 0)
        throw new Error("Cannot render a picture without optimized variants.");
    ...
    if (!fallback)
        throw new Error("Cannot select an image fallback.");
...
export async function image(sourcePath, options) {
