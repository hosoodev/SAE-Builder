...
test("OG SVG supports site-specific line lengths and truncates the final line", () => {
  const template = `<svg xmlns="http://www.w3.org/2000/svg"><text>{{subtitleLine1}}|{{subtitleLine2}}|{{subtitleLine3}}</text></svg>`;
  const svg = renderOgSvg({
    title: "Title",
    subtitle: "1234567890123456789012345678901234567890",
  }, {
    template,
    subtitleCharactersPerLine: 10,
  });
