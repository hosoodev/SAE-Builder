...
export function normalizeSlug(input) {
    ...
    if (input !== input.trim())
        failSlug(input, "surrounding whitespace is not allowed");
    if (!input.startsWith("/"))
        failSlug(input, "it must start with '/'");
    if (input.includes("?") || input.includes("#") || input.includes("\\")) {
        failSlug(input, "query strings, fragments, and backslashes are not allowed");
    }
    if (CONTROL_CHARACTER.test(input))
        failSlug(input, "control characters are not allowed");
    if (ENCODED_PATH_SEPARATOR.test(input))
        failSlug(input, "encoded path separators are not allowed");
    let decoded;
    try {
    ...
    if (CONTROL_CHARACTER.test(decoded))
        failSlug(input, "encoded control characters are not allowed");
...
export function isExplicitFileSlug(slug) {
...
export function outputPathForSlug(outputRoot, slug, trailingSlash = true) {
    const normalized = normalizeSlug(slug);
    ...
    const target = normalized === "/"
        ...
        : isExplicitFileSlug(normalized)
            ...
            : trailingSlash
                ...
                : path.resolve(root, ...routeSegments.slice(0, -1), (routeSegments.at(-1) ?? "") + ".html");
    const relativeTarget = path.relative(root, target);
    if (relativeTarget === "" || relativeTarget.startsWith("..") || path.isAbsolute(relativeTarget)) {
        throw new RouteError("OUTPUT_ROOT_ESCAPE", `The route ${JSON.stringify(normalized)} resolves outside the output root`);
    }
    return target;
}
export function createPageRoute(outputRoot, slug, trailingSlash = true) {
    const normalized = normalizeSlug(slug);
    return {
        slug: normalized,
...
export function assertUniqueSlugs(entries, getSlug, getLabel = () => "content entry") {
    const seen = new Map();
    for (const entry of entries) {
        const normalized = normalizeSlug(getSlug(entry));
        const previous = seen.get(normalized);
        if (previous !== undefined) {
            throw new RouteError("DUPLICATE_SLUG", `Duplicate normalized slug ${JSON.stringify(normalized)}: ${previous} and ${getLabel(entry)}`);
        }
        seen.set(normalized, getLabel(entry));
    }
}
export function resolveCanonical(siteUrl, slug, explicitCanonical) {
    let site;
    try {
    ...
    try {
        ...
        throw new RouteError("INVALID_CANONICAL", `Invalid canonical URL: ${JSON.stringify(explicitCanonical)}`);
    }
