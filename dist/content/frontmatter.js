...
export function validateFrontMatter(value, sourcePath) {
    const result = coreFrontMatterSchema.safeParse(value);
    if (!result.success)
        throw new FrontMatterError(sourcePath, result.error.issues);
    return result.data;
}
export function parseFrontMatter(source, sourcePath) {
    ...
    try {
        ...
        const location = sourcePath === undefined ? "content" : sourcePath;
        throw new Error(`Unable to parse front matter in ${location}: ${error instanceof Error ? error.message : String(error)}`, { cause: error });
    }
    return {
